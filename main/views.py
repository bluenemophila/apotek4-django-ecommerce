...
from .models import Banner,Category,Brand,Product,ProductAttribute,CartOrder,CartOrderItems,ProductReview,Wishlist,UserAddressBook,Coupon
...
def product_list(request):
	...
	data=Product.objects.all().order_by('id')[:12]
...
def signup(request):
	if request.method=='POST':
		...
		messages.warning(request,"Sign Up gagal. Informasi tidak valid.")
	form=SignupForm()
	...
	if 'cartdata' in request.session:
		...
		if request.method == 'POST':
			...
			try :
				...
				if order.coupon:
					messages.warning(request, 'Kode kupon valid.')
				
				if total_amt < coupon_obj.minimum_amount:
					messages.warning(request, f"Total belanja harus lebih dari {coupon_obj.minimum_amount}.")
				
				if coupon_obj.is_expired:
					messages.warning(request, f"Kupon sudah kadaluarsa.")
